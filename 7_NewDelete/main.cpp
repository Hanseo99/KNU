...
{
	// 정적 메모리 할당
	// 정적으로메모리를 할당하면 컴파일 시 Stack 영역에 할당되며, 함수를 빠져 나갈 때 소멸된다.
	// 프로그램 컴파일 시 Stack에 얼마만큼의 크기로 할당을 해야 하는 지 결정되기 때문에, 컴파일 이후 크기를 변경할 수 없다.
	...
	//// heap는 프로그래머에 의해 할당(new) 소멸(delete)된다.
	...
	//// new & delete
	//// new 연산자로 생성 시 반드시 delete 연산자로 해제 해주어야 한다.
	//// new 연산자로 배열 생성 시 delete[] 연산자로 배열임을 명시 해주어야 한다.
	...
	*a = *b;// 깊은 복사 : a가 가리키고 있는 값을 b가 가리키고 잇는 값으로 수정한다.
