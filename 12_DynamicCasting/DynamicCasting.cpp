...
public:
	...
	// 가상 함수
	// virtual 키워드를 함수 앞에 붙힌다
	// 자식 클래스에ㅔ서 함수를 override를 해서 재구현 가능
	// 가상 함수가 되면 RTTI정보를 가리키는 포인터 변수 공간 8Byte(x64기준)할당
	// RTTI 포인터는 해당 클래스의 정보를 가리키게 됨
	// 런타임 중 동적으로 할당
	// dynamic_cast 시 클래스의 크기를 참조하는 게 아니라 추가로 할당 된 RTTI 포인터의 클래스 정보를 참조
	virtual void printX()
	{
		printf("X = %d\n", m_X);
	}
...
int main()
{
	...
	for (CBase* pTemp : pBase)
	{
		...
		// (pTemp) : 캐스팅(형변환)할 객체
		// CChild1* pChild : 형변환 결과 반환값(캐스팅 성공 시 캐스팅 된 객체 반환,캐스팅 실패 시 nullptr 반환) 
