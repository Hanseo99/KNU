...
struct stTest
{
	...
	stTest(string name = "", int math = 0, int eng = 0, int lang = 0) ://값을 0으로 초기화하여 기본값을 0으로 만든다.
	...
	void SetData(string& str)
	{
		...
		{
			line.erase(remove(line.begin(), line.end(), ' '), line.end());// 맨 앞에 띄어쓰기를 지움

			int index = line.find('='); // '='를 기준으로 인덱스를 결정
			string leftStr = line.substr(0, index); // 0부터 인덱스(=)앞 까지를 잘라서 leftStr로 설정
			string rightStr = line.substr(index + 1, line.length() - index);// 인덱스(=)뒤 부터 끝까지 잘라서 rightStr로 설정
	...
	*/
	...
	if (readFile.is_open())
	{
		while (!readFile.eof())
		{
			...
			stTest newData;
			newData.SetData(str);
			vecTest.push_back(newData);//배열에 값을 집어넣음
		}
		readFile.close();
	}
